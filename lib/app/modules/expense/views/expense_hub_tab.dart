...
class ExpenseHubTab extends StatefulWidget {
  const ExpenseHubTab({super.key});

  @override
  State<ExpenseHubTab> createState() => _ExpenseHubTabState();
}

class _ExpenseHubTabState extends State<ExpenseHubTab>
  ...
  void initState() {
    ...
    _tabController = TabController(
      length: 3,
      vsync: this,
      initialIndex: dashboard.expenseTabIndex.value,
    );

    // Listen to tab index changes from controller
    ever(dashboard.expenseTabIndex, (index) {
    ...
    _tabController.addListener(() {
      if (!_tabController.indexIsChanging) {
        dashboard.expenseTabIndex.value = _tabController.index;
      }
    });
  }
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            children: const [IncomesTab(), ExpensesTab(), HistoryTab.account()],
          ),
        ),
      ],
    );
